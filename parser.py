...
def get_uzmovi_info(url, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            # Flexible iframe regex supporting any uzdown CDN domain or subdomain variant
            ...
            req2 = urllib.request.Request(iframe_url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'})
            iframe_html = urllib.request.urlopen(req2, timeout=10).read().decode('utf-8')

            # Flexible m3u8 matching with both quote types single (') or double (")
            m3u8_match = re.search(r"file:\s*['\"]([^'\"]+)['\"]", iframe_html, re.IGNORECASE)
            if not m3u8_match:
                # Fallback search for any m3u8 URL in iframe_html
                m3u8_fallback = re.search(r'["\'](https?://[^"\']+\.m3u8[^"\']*)["\']', iframe_html, re.IGNORECASE)
                if m3u8_fallback:
                    m3u8_url = m3u8_fallback.group(1)
                else:
                    return url, None, "m3u8 manba ssilkasi topilmadi."
            else:
                m3u8_url = m3u8_match.group(1)

            return url, {"title": title_clean, "folder": os.path.join("uzmovi", folder_name), "source_url": m3u8_url}, None
